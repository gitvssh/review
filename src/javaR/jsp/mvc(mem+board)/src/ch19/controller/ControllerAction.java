...
public class ControllerAction extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException{
		...
		String pros=path+config.getInitParameter("proFile");//CommandPro.properties
		...
		while(keyItor.hasNext()){//자료가 있는 동안 반복
			...
			try{
				...
				map.put(key, commandObject);///map에 key,value로 넣는다 
                //모든 Action을 클래스 객체로 만들어 map 넣었음	
				
			}catch(Exception ex3){
				System.out.println("proeperty 파일내용을 클래스 객체로 만드는 중 예외 발생 "+ex3);
			}//catch end 
		}//while end
	}//init() end---
	...
	private void reqPro(HttpServletRequest request,HttpServletResponse response) 
		...
		try{
			...
			commandAction=(CommandAction)map.get(uri);//   /ch19/list.do 에 해당하는 객체를 map에서 얻는다 
